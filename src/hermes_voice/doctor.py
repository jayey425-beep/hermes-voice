...
from .utils import current_python_path, hermes_config_path, hermes_python_path, module_exists_in_python, which
...
def build_fix_suggestion(check_name: str, runtime: str, python_path: str | None, detail: str = "") -> str:
...
def _module_fix_command(check_name: str, runtime: str, python_path: str | None) -> str | None:
...
def run_doctor(runtime: str = "hermes", profile: str = "default") -> DoctorReport:
    ...
    runtime_label = f"{'Hermes runtime' if runtime == 'hermes' else 'Wrapper runtime'} [{profile}]"
    source = runtime_source_label(runtime)
    ...
    report.checks.append(CheckResult(
        ...
        hint=build_fix_suggestion("runtime_python", runtime, active_python, active_python or "unable to resolve runtime interpreter"),
        source=source,
    ))
    ...
    for mod in VOICE_PY_MODULES:
        ...
        if not ok and mod == "faster_whisper" and is_python314_wrapper(active_python, runtime):
    ...
    report.checks.append(CheckResult(
        name="voice_env",
        ok=True,
        detail=", ".join(env_present) if env_present else "no cloud voice env keys set",
    ...
    report.checks.append(CheckResult(
        ...
        ok=voice_enabled,
        detail="voice block present" if voice_enabled else "voice block missing or empty",
...
def apply_doctor_fixes(runtime: str = "hermes", profile: str = "default") -> list[FixAction]:
    ...
    for check in report.failed_checks:
        ...
        if not command:
            actions.append(FixAction(check.name, check.source, "", True, "no automatic fix available"))
            continue
        if check.name == "py:faster_whisper" and is_python314_wrapper(python_path, runtime):
            actions.append(FixAction(check.name, check.source, command, True, PYTHON314_AV_NOTE))
            continue
